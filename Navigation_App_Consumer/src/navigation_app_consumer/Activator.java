package navigation_app_consumer;
...
public class Activator implements BundleActivator {
    ...
    public void start(BundleContext context) throws Exception {
        ...
        if (densityRef != null && speedRef != null && weatherRef != null && transitRef != null) {
            ...
            suggestNavigationRoute(density, speed);
        } else {
            System.out.println("Required services for Navigation App are not available.");
        }
    }
    
    private void suggestNavigationRoute(int density, double speed) {
        System.out.println("\nSuggesting Navigation Route:");
        if (density > 70) {
            System.out.println("High congestion detected. Suggesting alternative route with lighter traffic.");
        } else {
            System.out.println("Traffic conditions are normal. Using the default navigation route.");
        }
    }
    
    @Override
    public void stop(BundleContext context) throws Exception {
        System.out.println("Navigation App Consumer stopped.");
    }
}
